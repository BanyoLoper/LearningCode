 ...
 *
 * The user email comes from the Cloudflare Zero Trust header.
 * Binding "DB" must be a D1 database configured in wrangler.toml / Pages settings.
 */

export async function onRequestGet(context) {
  const email =
    context.request.headers.get('Cf-Access-Authenticated-User-Email') ??
    context.env.DEV_EMAIL ??
    null;
  if (!email) {
